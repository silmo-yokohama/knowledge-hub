...
1. Glob ツールで `03.Learnings/**/*` を確認し、既存の学習ログリストを取得する（重複チェック用）
2. ToolSearch ツールで NotebookLM MCP ツール群を検索する（キーワード: `notebooklm`）
   - ツールが見つからない場合 → 「NotebookLM MCPフォールバック」セクションに従う
3. ユーザーの興味領域・学習中の分野・実務文脈は、対話の中で把握する（Step 2 のヒアリングで確認する）
...
3. **応用分析クエリ**: 対話で把握した興味領域・実務文脈に基づいた応用分析
...
| Step 3-3 | `notebook_query` × 3回 | 要約・構造化・応用分析 |
...
- レポートの「実務への応用」セクションは、対話で把握した実務文脈を活用して具体的にする
